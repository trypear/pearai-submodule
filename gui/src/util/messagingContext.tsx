...
interface MessagingContextValue {
  ...
  typedSend: <MessageType extends string, PayloadType = any, ResponseType = any>(
...
export const MessagingProvider: React.FC<MessagingProviderProps> = ({ 
  ...
  const [pendingResponses, setPendingResponses] = useState<
    Record<string, { resolve: (value: any) => void; reject: (reason?: any) => void }>
  >({});
    ...
    if (waitForResponse) {
      return new Promise<ResponseType>((resolve, reject) => {
        ...
        setTimeout(() => {
          setPendingResponses(prev => {
            if (prev[messageId]) {
              prev[messageId].reject(new Error(`Message ${messageType} timed out`));
              const updated = { ...prev };
  ...
  const typedRegister = useCallback(<MessageType extends string, PayloadType = any>(
    messageType: MessageType,
    callback: (message: WebMessageOutgoing<PayloadType>) => void
  ) => {
