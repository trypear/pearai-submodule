...
const firstPearAICreatorLaunchFlag = path.join(
  pearAISettingsDir,
  "firstLaunchCreator.flag",
);
export const isFirstPearAICreatorLaunch = !fs.existsSync(
  firstPearAICreatorLaunchFlag,
);
...
async function copyDirectoryRecursiveSync(
  source: string,
  destination: string,
  exclusions: string[] = [],
) {
  ...
  try {
    ...
    if (!exists) {
      await fs.promises.writeFile(
        flagFile,
        `This is the first launch flag file for ${productName}`,
      );
    }
  } catch (error) {
